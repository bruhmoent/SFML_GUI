...
template <typename T>
void VerticalMenu<T>::addTextItem(const T& item)
{
	sf::Text text;
	text.setFont(m_font);
...
template <typename T>
void VerticalMenu<T>::staticDisplay(sf::RenderWindow& window)
{
...
template <typename T>
void VerticalMenu<T>::setPosition(float x, float y)
{
	m_position.x = x;
	m_position.y = y;
}
...
//Handle hovering
template <typename T>
void VerticalMenu<T>::handleHover(const sf::RenderWindow& window)
{
...
//Get selected index
template <typename T>
int VerticalMenu<T>::getSelectedIndex() const
{
...
//Get background
template <typename T>
sf::RectangleShape VerticalMenu<T>::getBackground() const
{
	return m_background;
}
